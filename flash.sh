...
3. Flash VBMETA
4. Use adb Sideload
5. Use adb Sideload and Reboot to recovery
6. Check adb devices
7. Check fastboot devices
8. Reboot to System
9. Reboot to Recovery
10. Reboot to Bootloader
11. Reboot to Fastboot
12. Flash MIUI Stock ROM (Fastboot)
...
case $flasher in
  "1")
    ...
    files=("boot.img" "./platform-tools/fastboot" "./platform-tools/adb")
    ...
    echo -e "\033[0;32m Flashing Boot \033[0m";
    ./platform-tools/fastboot flash boot boot.img

     read -p "Do you want to flash 'recovery.img' file? Type 'y' to flash, any key to skip: " recovery;

    if [ "$recovery" = "y" ] || [ "$recovery" = "yes" ] || [ "$recovery" = "Y" ] || [ "$recovery" = "YES" ]; then
      echo -e "\033[0;33m make sure 'recovery.img' file exist in the root directory. \033[0m";
      echo -e "\033[0;32m Flashing recovery.img \033[0m";
      ./platform-tools/fastboot flash recovery recovery.img
    else
      echo -e "\033[0;32m Skipping Flashing recovery.img \033[0m";
    fi
    ...
    read -p "Type 'y' to reboot to recovery mode, any key to skip: " rmode;

    if [ "$rmode" = "y" ]; then
      echo -e "\033[0;32m Rebooting to Recovery. \033[0m";
      ./platform-tools/fastboot reboot recovery
    else
      echo -e "\033[0;32m Skipping Rebooting to Recovery. \033[0m";
    fi
  ...
  "2")
    ...
    read -p "Type 'y' to reboot to recovery mode, any key to skip" rmode1;
